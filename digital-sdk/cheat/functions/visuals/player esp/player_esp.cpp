...
void c_player_esp::draw()
{
	for (int i = 0; i < g_sdk.m_interfaces.m_globals->m_max_clients; i++)
	{
		...
		if (g_cfg.m_esp.m_box)
		    render_box(bbox, entity);

		if (g_cfg.m_esp.m_name)
		    render_name(bbox, entity);
		
		if (g_cfg.m_esp.m_healthbar)
...
void c_player_esp::render_health_bar(const RECT bbox, const c_base_player* player) const
{
	...
	auto color = c_color(0, 255, 0);

	if (hp <= 100 && hp >= 81)
		color = c_color(0, 255, 0);
	else if (hp <= 80 && hp >= 50)
		color = c_color(0, 140, 0);
	else if (hp <= 49 && hp >= 25)
		color = c_color(0, 100, 0);
	else if (hp <= 24)
		color = c_color(145, 9, 9);
	...
	c_render::get()->rect_filled(bbox.left - 5.0f, bbox.top + (colored_bar_max_height - colored_bar_height),
	                             bbox.left - 3.0f, bbox.top + colored_bar_max_height, color);

	if (hp < 100)
	    c_render::get()->text(g_sdk.m_fonts.m_esp, ImVec2(bbox.left - 22, bbox.top + (colored_bar_max_height - colored_bar_height)),
	                             std::to_string(hp), c_color::white, false, true);
}
